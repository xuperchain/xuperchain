...
/*func TestGenerateTx(t *testing.T) {
	...
	isOnChain := ledger.IsTxInTrunk(tx.Txid)
	t.Log("xuper tx is on chain ??? ", isOnChain)

	txReq := &pb.TxData{}
	txReq.Bcname = "xuper-chain"
	txReq.FromAddr = Users["bob"].Address
	txReq.FromPubkey = Users["bob"].Pubkey
	txReq.FromScrkey = Users["bob"].PrivateKey
	txReq.Nonce = "nonce"
	txReq.Timestamp = time.Now().UnixNano()
	txReq.Desc = []byte("hello")
	txReq.Account = []*pb.TxDataAccount{
		{Address: Users["alice"].Address, Amount: "1", FrozenHeight: int64(0)},
	}
	tx, genTxErr := utxoVM.GenerateTx(txReq)
	if genTxErr != nil {
		t.Log("gen_tx error ", genTxErr.Error())
	}
	txReq.Account = []*pb.TxDataAccount{
		{Address: Users["alice"].Address, Amount: "-1", FrozenHeight: int64(0)},
	}
	...
	txReq.Account = []*pb.TxDataAccount{
		{Address: Users["alice"].Address, Amount: "100000", FrozenHeight: int64(0)},
	}
	utxoVM.GenerateTx(txReq)

	ti := &testInterface{}
	utxoVM.RegisterVM("testInterface", ti, global.VMPrivRing0)
	txDesc := contract.TxDesc{
		Module: "testInterface",
		Method: "CreateChainBlock",
	}
	b, _ := json.Marshal(txDesc)
	ret, status := utxoVM.isSmartContract(b)
	t.Log("isSmartContract ret ", ret)
	t.Log("isSmartContract status ", status)
	utxoVM.isSmartContract([]byte("aha"))
	// utxoVM.UnRegisterVM("testInterface", global.VMPrivRing0)
	// utxoVM.makeExtensionForFailure(tx)
	// utxoVM.TxExtensionGenerate(ti, ret, tx)
	...
	txReq.Desc = b
	txReq.Account = []*pb.TxDataAccount{
		{Address: Users[to].Address, Amount: amount, FrozenHeight: int64(0)},
		{Address: FeePlaceholder, Amount: "3"},
	}
	//txlist, _, ecdsaPk, err := utxoObj.GenerateTx("alice", "bob", t, amount, preHash, desc, 0)
	tx1, err := utxoVM.GenerateTx(txReq)
	t.Log("tx1 ", tx1)
	t.Log("preHash ", preHash)

	// 这里VerifyTx fail
	verifyOK, vErr := utxoVM.ImmediateVerifyTx(tx1)
	if !verifyOK || vErr != nil {
		t.Error("verify tx fail", vErr.Error())
	}

	errDo := utxoVM.DoTx(tx1)
	if errDo != nil {
		t.Log("DoTx error ", errDo.Error())
	}
	txlist, _ := utxoVM.GetUnconfirmedTx(true)
	awardTx, minerErr := utxoVM.GenerateAwardTx([]byte("miner-1"), "1", []byte("award,onyeah!"))
	if minerErr != nil {
		t.Log("GenerateAwardTx error ", minerErr.Error())
	}
	txlist = append(txlist, awardTx)

	ecdsaPk, _ := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	pendingBlock, _ := ledger.FormatBlock(txlist, []byte("miner-1"), ecdsaPk, 123456789, 0, 0, preHash, utxoVM.GetTotal())
	txs, _, err := utxoVM.TxOfRunningContractGenerate(txlist, pendingBlock, nil, true)
	t.Log("txs ", txs)

	t.Log("Type txDesc ", reflect.TypeOf(txDesc))

	// test for MakeContractTransferTx
	bobBalance, _ := utxoVM.GetBalance(BobAddress)
	aliceBalance, _ := utxoVM.GetBalance(AliceAddress)
	t.Log("bob balance ", bobBalance)
	t.Log("alice balance ", aliceBalance)
