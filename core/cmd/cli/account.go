...
func NewAccountCommand(cli *Cli) *cobra.Command {
	...
	c.cmd = &cobra.Command{
		Use:   "account",
		Short: "Operate an account or address: balance|new|newkeys|split|merge|list-utxo|lock|unlock.",
	}
	...
	c.cmd.AddCommand(NewAccountLockCommand(cli))
	c.cmd.AddCommand(NewAccountUnLockCommand(cli))
	return c.cmd
}
