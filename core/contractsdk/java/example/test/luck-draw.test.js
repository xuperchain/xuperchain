...
var codePath = "../luck-draw/target/luck-draw-0.1.0-jar-with-dependencies.jar"
var lang = "java"
var type = "native"
...
Test("LuckDraw", function (t) {
    ...
    var resp = c.Invoke("startLuckDraw", { "seed": "1000" }, { "account": "xchain" })
    assert.equal(resp.Message, "")
    assert.equal(resp.Status, 200)
    var resp = c.Invoke("getResult", {})
    assert.equal(resp.Message, "")
    assert.equal(resp.Status, 200)
    console.log("luck_user:", resp.Body) // luck user is the same as we always start with the same seed
