...
function StoreShortContent(){
    ...
    resp = c.Invoke("StoreShortContent",{"user_id":"user1","topic":"topic1","title":"title1","content":"content1"})
...
function beforeTest(){
    ...
    for(var i = 0;i<2;i++){
        for(var j = 0;j<2;j++){
            for(var k = 0;k<2;k++){
                resp = c.Invoke("StoreShortContent",{"user_id":users[i],"topic":topics[j],"title":titles[k],"content":"content"+i+j+k})}
        }
    }
    return c
}
