...
// Table implements table storage, with pb::Message as a record, supporting
// rowkey which should be unique, and multiple indeies. Layout on KV:
...
// (3) store the index, with i as suffix, you can range_quey {index i} to get
// all the rows indexed by {index i} (4) {rowkey}{i} is also unique, being used
// to find the index when deleting
template <typename T>
struct Table {
public:
    Table(xchain::Context* ctx, const std::string&);
    virtual ~Table();
    // find a row by key, which is rowkey, if return true, t will bring back the
    // deserialized object.
...
private:
    ...
    bool write_index(const std::string& rowkey, const std::string& index_str,
                     int i);
    bool delete_index(const std::string& rowkey, const std::string& index_str,
                      int i);
    std::string make_key(const std::string& prefix, const std::string& value);
    std::string make_index(
        std::initializer_list<PairType> input, bool key,
