...
template <typename T>
std::string Table<T>::make_key(const std::string& prefix,
...
bool Table<T>::find(std::initializer_list<PairType> input, T* t) {
    ...
    const std::string& key =
        make_index(input, false, split(tmp.rowkey_str(), ","));
...
template <typename T>
bool Table<T>::write_index(const std::string& rowkey,
                           const std::string& index_str, int idx_no) {
    ...
    std::string idx_key =
        make_key(PREFIX_INDEX_DEL, rowkey + std::to_string(idx_no));
...
template <typename T>
bool Table<T>::delete_index(const std::string& rowkey,
                            const std::string& index_str, int idx_no) {
    std::string idx =
        make_key(PREFIX_INDEX_DEL, rowkey + std::to_string(idx_no));
    std::string value;
...
template <typename T>
std::vector<std::string> Table<T>::split(const std::string& input,
...
template <typename T>
std::string Table<T>::make_index(std::initializer_list<PairType> input,
                                 bool key,
...
template <typename T>
std::unique_ptr<TableIterator<T>> Table<T>::scan(
    std::initializer_list<PairType> input) {
    auto idx = make_index(input, true);
    T t;
    if (!t.has(idx)) {
        auto it =
            std::unique_ptr<TableIterator<T>>(new TableIterator<T>(_ctx, ""));
...
TableIterator<T>::TableIterator(xchain::Context* ctx, std::string idx)
    : Iterator(idx, idx + KEY_END, ITERATOR_BATCH_SIZE),
      _ctx(ctx),
      _index(idx) {}
