...
public:
    bool checkArg(xchain::Context* ctx, const std::string& key,
                  std::string& value) {
        value = ctx->arg(key);
        if (value.empty()) {
    ...
    void split(std::vector<std::string>& str_sets, const std::string& str,
    ...
    bool parseValidates(xchain::Context* ctx, xchain::json* jValidatesObject) {
        ...
        *jValidatesObject = xchain::json::parse(buffer);
        ...
        if (proposersIter == jValidatesObject->end() ||
            !(*proposersIter).is_array() || (*proposersIter).empty() ||
            !(*proposersIter).size()) {
    ...
    bool findItem(xchain::Context* ctx, xchain::json& jObject,
                  const std::string& targetStr, xchain::json::iterator* iter) {
        *iter = std::find_if(jObject.begin(), jObject.end(),
                             jvectorFinder(targetStr));
    ...
    void initialize() {
        ...
        if (!checkArg(ctx, "addresss", addresss) ||
            !checkArg(ctx, "neturls", neturls)) {
            return;
        }
        ...
        for (int i = 0; i < address_sets.size(); ++i) {
            xchain::json jItem = {{"Address", address_sets[i]},
        ...
        if (validatesStr.empty() ||
            !ctx->put_object(VALIDATES_KEY, validatesStr)) {
    ...
    void add_validate() {
        ...
        if (findItem(ctx, jValidatesObject["proposers"], address,
                     &proposerIter)) {
            ctx->error("Proposer has exist");
            return;
        }

        xchain::json jItem = {{"Address", address}, {"PeerAddr", neturl}};
