...
func GenCandidateBallotsPrefix() string {
	return pb.ConsPoaPrefix + "_candidate_ballots_"
}
...
func GetCandidateInfoPrefix() string {
	return pb.ConsPoaPrefix + "_candidate_info_"
}
...
func GenNominateRecordsPrefix(addr string) string {
	return pb.ConsPoaPrefix + "_nominate_record_" + addr
}
...
func genCandidateNominatePrefix() string {
	return pb.ConsPoaPrefix + "_candidate_nominate_"
}
...
func GenCandidateVotePrefix(addrCandi string) string {
	return pb.ConsPoaPrefix + "_candidate_vote_" + addrCandi + "_"
}
...
func GenVoteCandidatePrefix(addrVoter string) string {
	return pb.ConsPoaPrefix + "_vote_candidate_" + addrVoter + "_"
}
...
func genTermCheckKeyPrefix(version int64) string {
	return pb.ConsPoaPrefix + fmt.Sprintf("_%d", version)
}
...
func genRevokeCandidateKey(address, txid string) string {
	...
	return pb.ConsPoaPrefix + "_candidate_revoke_" + baseKey
}

// 生成撤销存储,避免重复撤销
func genRevokeKey(txid string) string {
	return pb.ConsPoaPrefix + "_revoke_" + txid
}
