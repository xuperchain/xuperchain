...
class XRC01 {
    public: 
        XRC01(xchain::Context* ctx): _token(ctx, "token"), _asset_info(ctx, "asset_info"), _authorize_info(ctx, "_authorize_info") {
            _ctx = ctx;
        }
    ...
    public:
        // following interfaces declare the main interfaces of XRC01 multi token standard
        // issue a token 
        bool issue(XRC01::token token);
        // authorization token to someone
        bool authorization(const std::string& to, uint64_t token_id,  uint64_t amount);
        // withdraw authorization token from someone
        bool withdraw_authorization(const std::string& from, uint64_t token_id,  uint64_t amount);  
        ...
        // authorize transfer token from someone to someone, the caller need to be authorized by from
        bool transfer_from(const std::string& from, const std::string& to, uint64_t token_id,  uint64_t amount);
        // get_balance return the token balance of given account
        bool get_balance(const std::string& account, uint64_t token_id, uint64_t* balance);
        // get_authorized return the token have authorized to others
        bool get_authorized(const std::string& account, uint64_t token_id, uint64_t* authorized);
        ...
        bool authorize_infos(const std::string& account, uint64_t token_id, std::vector<XRC01::authorize_info>& authorize_infos);
        // authorized_infos return tokens the account be authorized
        bool authorized_infos(const std::string& account, uint64_t token_id, std::vector<XRC01::authorize_info>& authorize_infos);
};