/*
* XRC-01 is a multi token standard used for smart contracts on the XuperChain blockchain. 
* It's can include combination of fungible and non-fungible tokens in one contract.
*/
#include "xrc01.h"
...
std::string make_authorize_info_id(const std::string from, const std::string to, uint64_t token_id) {
...
bool XRC01::issue(XRC01::token token) {
    ...
    std::string  asset_info_id = make_asset_info_id(token.issue_account(), token_id);
...
bool XRC01::authorization(const std::string& to, uint64_t token_id,  uint64_t amount){
    XRC01::token token;
    ...
    if (caller == to) {
        printf("Authorization failed, can not authorization to self, \
    ...
    if (! _asset_info.find({{"id", asset_info_id}}, &asset_info)){
        printf("Authorization failed, you do not have token, \
    ...
    if (asset_info.amount() - asset_info.authorized() < amount) {
        printf("Authorization failed, asset remained not enough, \
         from=%s, to=%s, token_id=%llu, amount=%llu, authorized=%llu \n",
          caller.c_str(), to.c_str(), token_id, amount, asset_info.authorized()); 
        return false;
    }   
    ...
    if (!_asset_info.update(asset_info)) {
        printf("Authorization failed, update asset_info error,\
         from=%s, to=%s, token_id=%llu, amount=%llu \n", caller.c_str(), to.c_str(), token_id, amount); 
        return false;
    }

    // Update authorize_info
    std::string  authorize_info_id = make_authorize_info_id(caller, to, token_id);
    if (! _authorize_info.find({{"id", authorize_info_id}}, &authorize_info)){
        ...
        if (!_authorize_info.put(authorize_info)) {
            printf("Authorization failed, store authorization info failed, \
                from=%s, to=%s, token_id=%llu, amount=%llu \n", caller.c_str(), to.c_str(), token_id, amount); 
            return false;
        }
    } else {
        if (!xchain::safe_assert( amount + authorize_info.amount() >= authorize_info.amount())) {
            printf("safemath assert failed");
            return false;
        }   
        authorize_info.set_amount(amount + authorize_info.amount());
        if (!_authorize_info.update(authorize_info)) {
            printf("Authorization failed, update authorization info failed, \
                from=%s, to=%s, token_id=%llu, amount=%llu \n", caller.c_str(), to.c_str(), token_id, amount); 
            return false;
        }
    }

    printf("Authorization success, from=%s, to=%s, token_id=%llu, amount=%llu \n", caller.c_str(), to.c_str(), token_id, amount); 
    return true;
}

bool XRC01::withdraw_authorization(const std::string& to, uint64_t token_id,  uint64_t amount) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued. \n", token_id);
        return false;                            
    ...
    std::string asset_info_id = make_asset_info_id(caller, token_id);
    if (! _asset_info.find({{"id",asset_info_id}}, &asset_info)){
        printf("Withdraw_authorization failed, you do not have token, token_id=%llu \n", token_id); 
        return false;
    }

    std::string authorize_info_id = make_authorize_info_id(caller, to, token_id);
    if (! _authorize_info.find({{"id", authorize_info_id}}, &authorize_info)){
        printf("Withdraw_authorization failed, you haven't authorized token_id=%llu to %s \n", token_id, to.c_str());
        return false; 
    }

    if (authorize_info.amount() < amount) {
        printf("Withdraw_authorization failed, you haven't authorized enough token \n");
        return false;
    }
    ...
    if (authorize_info.amount() - amount == 0) {
        if (! _authorize_info.del(authorize_info)) {
            printf("Withdraw_authorization failed, del authorize_info error \n");
            return false;
        }
    } else {
        if (! _authorize_info.update(authorize_info)) {
            printf("Withdraw_authorization failed, store authorize_info error \n");
            return false;
        }
    }
...
bool XRC01::transfer(const std::string& to, uint64_t token_id,  uint64_t amount) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued. \n", token_id);
        return false;                            
    ...
    if (caller == to || amount < 1) {
        printf("transfer_from failed, at least transfer 1 token and from can not equal with to. \n");
        return false;         
    }
    ...
    if (!xchain::safe_assert(asset_info_caller.amount() >= asset_info_caller.authorized())) {
        printf("safemath assert failed");
        return false;
    } 
    if (asset_info_caller.amount() - asset_info_caller.authorized() < amount) {
        printf("Transfer failed, you do not have enough token, token_id=%llu \n",token_id); 
        return false;
    }
...
bool XRC01::transfer_from(const std::string& from, const std::string& to, uint64_t token_id,  uint64_t amount){
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued. \n", token_id);
        return false;                            
    ...
    if (!_authorize_info.find({{"id", authorize_info_id}}, &authorize_info)) {
        printf("Transfer_from failed, from=%s to=%s, caller can not be authorized. \n", from.c_str(), to.c_str());
        return false;
    }
...
bool XRC01::get_balance(const std::string& account, uint64_t token_id, uint64_t* balance) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued. \n", token_id);
        return false;                            
...
bool XRC01::get_authorized(const std::string& account, uint64_t token_id, uint64_t* authorized) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued. \n", token_id);
        return false;                            
...
bool XRC01::authorize_infos(const std::string& account, uint64_t token_id, std::vector<XRC01::authorize_info>& authorize_infos) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued.\n", token_id);
        return false;                            
    ...
    auto it = _authorize_info.scan({{"from", account}, {"token_id", std::to_string(token_id)}});
    while (it->next()) {
        XRC01::authorize_info authorize_info;
...
bool XRC01::authorized_infos(const std::string& account, uint64_t token_id, std::vector<XRC01::authorize_info>& authorized_infos) {
    XRC01::token token;
    if (!_token.find({{"id", std::to_string(token_id)}}, &token)) {
        printf("Token with token_id=%llu haven't issued.\n", token_id);
        return false;                            
    ...
    auto it = _authorize_info.scan({{"to", account}, {"token_id", std::to_string(token_id)}});
    while (it->next()) {
        XRC01::authorize_info authorize_info;
        if (it->get(&authorize_info)) {
